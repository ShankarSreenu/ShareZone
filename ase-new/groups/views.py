    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if User.objects.get(username=form.cleaned_data['username']):
                groupuser = GroupUserTable.objects.create(group=group, user=User.objects.get(username=form.cleaned_data['username']), status=9)
                ntf = User.objects.get(username=form.cleaned_data['username']).notification_set.create(
                    ...
                    curr_user=request.user.username, type="Group")
                ntf.save()
